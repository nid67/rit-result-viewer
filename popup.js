...
window.addEventListener('load', () => {
  if (chrome && chrome.storage && chrome.storage.local) {
    chrome.storage.local.get(['savedRegNumber'], (result) => {
      if (result && result.savedRegNumber) {
        regNumberInput.value = result.savedRegNumber;
      }
    });
  }
});
...
loginForm.addEventListener('submit', async (e) => {
  ...
  if (chrome && chrome.storage && chrome.storage.local) {
    chrome.storage.local.set({ 'savedRegNumber': regNumber });
  }
  ...
  try {
    ...
    console.log('First 1500 chars:', loginPageHtml.substring(0, 1500));
    ...
    if (!tokenMatch || !tokenMatch[1]) {
      ...
      throw new Error('Could not find CSRF token in login page. The login page structure may have changed.');
    }
    
    csrfToken = tokenMatch[1];
    console.log('CSRF Token extracted successfully:', csrfToken.substring(0, 20) + '...');
    ...
    console.log('Response first 1000 chars:', responseHtml.substring(0, 1000));
    ...
    showStatus('✓ Login successful! Opening results page...', 'success');
    
    // Step 5: Open the results page in a new tab
    chrome.tabs.create({ 
      url: RESULT_PAGE_URL,
      active: true 
    });
